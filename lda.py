...
iter_num = 2
top_words = 20
#D:\pycharm\PythonWorkspace\PyLda\Data_process\a2.txt
wordmapfile  = './model/wordmap.txt'
trnfile = "./Data_process/a2.dat"
    ...
    def save_model(self):
        ...
        with open(self.modelfile_suffix+'.phi', 'w') as fphi:
            for x in xrange(self.K):
                ...
                fphi.write('\n')
        with open(self.modelfile_suffix+'.twords','w', encoding = 'utf-8') as ftwords:
            if self.top_words > self.dset.V:
                self.top_words = self.dset.V
            for x in xrange(self.K):
                print('Topic '+str(x)+'th:\n')
